...
> 本文档专门介绍 `MyCombo` 多选下拉勾选输入框。**与 `MyTable` 完全分离**：Excel 风格表格相关的内容请看 [my_table.md](my_table.md)。
...
```
┌─────────────────────────────────────┐
│  ☑ 全选（tristate checkbox）        │  ← 全选/取消全选（读 list 实际状态，不读 checkbox state 避免 tristate 循环）
├─────────────────────────────────────┤
│  🔍 搜索框（双向同步主输入框）      │  ← search_in_popup=True 时显示
├─────────────────────────────────────┤
│  ☐ Apple                            │
│  ☑ Banana                           │  ← _CheckListWidget
│  ☐ Cherry                           │
├─────────────────────────────────────┤
│  ⇅ 升序   ⇅ 降序  反选  重置      │  ← 底部左区（升序/降序只重排弹窗内可见项，不影响主表格/数据顺序）
│              确定  取消             │  ← 底部右区
└─────────────────────────────────────┘
```
...
弹窗打开时，**对当前选中项拍一份快照**。此后在弹窗内做的所有操作（勾选 / 反选 / 重置 / 全选 checkbox）**仅修改弹窗内的暂存状态**，**不立即**通过 `selectionChanged` 信号提交给外部。

何时生效：

| 操作 | 效果 |
|------|------|
| **确定** 按钮 | 暂存 → 真实选中，**发射** `selectionChanged`，关闭弹窗 |
| 鼠标 hover "仅筛此项"浮窗 | 暂存 → 真实选中，**发射** `selectionChanged`，关闭弹窗 |
| **取消** 按钮 / Esc | **丢弃**暂存，恢复快照，关闭弹窗。**不发射**信号 |

> 缓冲模式的设计意图：用户在弹窗内可以反复试（全选 → 反选 → 重置等），只有最后点"确定"才真正影响外部状态。避免"点一下就触发一堆回调"。

### 4.2 全选 QCheckBox（tristate）

- 三种状态：**全部未选**（□）、**部分已选**（▣）、**全部已选**（☑）
- 点击循环：**全部已选 ↔ 全部未选**（跳过中间态）
- 读取状态依据：遍历 **list 中可见项**的实际勾选状态，**不读 QCheckBox 自身的 state**——避免 `setTristate(True)` 的 3 态 cycle 与 `setCheckState` 冲突

### 4.3 底部按钮

| 按钮 | 作用范围 | 是否关闭面板 |
|------|----------|-------------|
| **升序 / 降序** | 当前**可见**项 → 按 label 字母/数字排序（**仅**重排弹窗内列表，不影响外部） | 否 |
| **反选** | 当前**可见**项 → 反转每项勾选状态（暂存） | 否 |
| **重置** | 当前**可见**项 → 全部**取消**勾选（暂存） | 否 |
| **确定** | 暂存 → 真实选中，发射信号 | **是** |
| **取消** | **丢弃**暂存，恢复快照 | **是** |

> 「作用于可见项」的统一语义意味着：在输入框中输入关键词 → 列表只显示匹配项 → 此时点"全选 checkbox"或"反选"等同于"批量操作所有匹配 XX 的项"。

关闭弹窗由 **确定**、**取消**、**Esc 键**、**点击外部** 触发，**不**由其他按钮触发——避免误操作打断批量选。

### 4.4 升序 / 降序说明

弹窗内的排序按钮**只影响弹窗中列表的显示顺序**。它们：
...
这意味着：输入 "B" 过滤 → 点升序 → 列表显示 "Banana, Berry" 而非 "Apple, Banana, Berry, Cherry"。
...
- 弹窗右上角实时显示 `已选 N 项`
- **弹窗只由确定 / 取消 / Esc / 外部点击关闭**，无 hover-leave 自动关闭逻辑
...
4. **弹窗只由确定/取消/Esc/外部点击关闭**。弹窗内子控件 NoFocus，`focusWidget()` 在点击勾选时会变成 `None`，如果按"失焦即关闭"会导致点一下就关。删除按钮不关闭弹窗——按钮只操作暂存状态，用户试完再点"确定"。同理，**不存在 hover-leave 自动关闭**，避免鼠标从列表移动到按钮时弹窗闪退。
...
弹出窗口（持久持有，show / hide）：
  _MultiSelectPopup (QFrame, Qt.Tool | Frameless)
   ...
   ┌─────────────────────────────────────┐
   │  ☑ 全选（tristate checkbox）        │
   ├─────────────────────────────────────┤
   │  🔍 (搜索框，search_in_popup 时)   │
   ├─────────────────────────────────────┤
   │  ☐ Apple                            │
   │  ☑ Banana                           │   ← _CheckListWidget
   │  ☐ Cherry                           │
  ...
  • 弹窗打开时拍一份"快照"（当前选中项拷贝）
  • 勾选/反选/重置/全选只改暂存状态（不触发 selectionChanged）
  • 确定 / hover "仅筛此项" → 暂存提交为真实选中 + 发信号
  • 取消 / Esc → 丢弃暂存，恢复快照

  "仅筛此项"浮窗（hover button）：
   挂载在 _CheckListWidget.viewport() 上（parent = viewport）
   避免鼠标从列表移入按钮时触发 viewport Leave 闪烁
...
#### (5) 弹窗按钮操作（缓冲模式 + 作用于可见项）

`apply_filter(text)` 通过 `QListWidgetItem.setHidden(True/False)` 隐藏不匹配项。

弹窗打开时，**所有操作（勾选 / 反选 / 重置 / 全选 checkbox）均在暂存副本上进行**，不触发 `selectionChanged`。

底部按钮作用范围：
- **升序 / 降序** — 重排**可见**项，不影响勾选状态，不关弹窗
- **反选** — 反转**可见**项勾选状态（暂存）
- **重置** — 清除**可见**项勾选（暂存）
- **确定** — 提交暂存 → 真实选中 → 发射信号 → 关闭弹窗
- **取消** — 丢弃暂存 → 恢复打开快照 → 关闭弹窗

全选 checkbox（tristate）根据**可见项实际勾选状态**计算：全部勾选 → ☑，部分勾选 → ▣，全部未选 → □。点击循环：☑ ↔ □（跳过 ▣）。
...
### 12.2 弹窗按钮统一为"确定/取消"缓冲模式

**现象**：原来按钮直接触发外部勾选状态变化，用户点击勾选后无法"反悔"。

**修复**：引入缓冲模式：

| 机制 | 说明 |
|------|------|
| 弹窗打开时拍快照 | 拷贝当前 `selected_values` 作为撤销锚点 |
| 全选 checkbox / 反选 / 重置 | 只改暂存，不发射信号，不关弹窗 |
| **确定** 按钮 | 暂存 → 真实选中，发射 `selectionChanged`，关闭 |
| **取消** 按钮 / Esc | 丢弃暂存，恢复快照，不发射信号，关闭 |

同时移除旧的 hover-leave 自动关闭逻辑（曾导致鼠标从列表移动到按钮时弹窗闪退）。
...
> 完。`MyTable` 相关的内容请看 [my_table.md](my_table.md)。
